...
var CSS = {
    ...
    toggleClass: function(b, a) { (CSS.hasClass(b, a) ? CSS.removeClass: CSS.addClass)(b, a)
        },
...
Subscriber.prototype.subscribe = function(b, a) { (this.events[b] || (this.events[b] = [])).push(a)
    };
...
Ajax.getXMLHttpRequest = function(f) {
    ...
    if (f == document.location.host) {
        ...
        if (Ajax.domains[f]) {
            ...
            c.src = document.location.protocol + "//crossdomain.html";
...
Ajax.prototype.send = function() {
    ...
    this.req.onreadystatechange = function() {
        if (this.req.readyState == 4) { (function() {
                var ex = null;
                try {
...
Navi.prototype.playSound = function(b, c) {
    ...
    if (a < this.nextSilence) { (function() {
            this.playSound(b, c)
            }).bind(this).defer(this.nextSilence - a)
        } else {
        this.nextSilence = a + Navi.soundLength;
/*        this.sound.playSound("//static.endoftheinter.net/navi/" + b + ".mp3"); (function() {
...
TopicManager.prototype.updateMessages = function(newCount) {
    ...
    if (maxMessages <= newCount) {
        ...
        }.bind(this)).send();++this.pendingUpdate
    }
...
TopicManager.prototype.updateBookmark = function() {
    ...
    function iterateRange(startContainer, startOffset, endContainer, endOffset, reverse, balanceNode, start, end, text) {
        var tmp,
        getNextNode = reverse ? getPrevSiblingDeep: getNextSiblingDeep;
        var endNode;
        ...
        while (node != endNode) {
            lastNode = node;
            if (tmp = reverse ? node.lastChild: node.firstChild) { (reverse ? end: start)(node);
                node = tmp
            } else {
    ...
    function getLLMLFromRange(range, balance) {
        ...
        iterateRange(range.startContainer, range.startOffset, range.endContainer, range.endOffset, false, balance, function(node) {
            ...
            if (node.tagName == "DIV" && node.className == "quoted-message") {--quoteDepth
            } else {
    ...
    QuickPost.prototype.onpreview = function() {
        if (Ajax("/async-post.php").data($M(DOM.serializeForm(this.root), {
            ...
            var toggle = function() {
                CSS.removeClass(this.root, "quickpost-preview"); [div, buttons].forEach(function(ii) {
...
ImageLoader.doneHandler = function(a) {
    if (a in ImageLoader.loading) {
        ...
        delete ImageLoader.loading[a];++ImageLoader.loadingSlots
    }
};
...
ImageLoader.prototype.load = function() {
    ...
    ImageLoader.loading[this.instance] = this;--ImageLoader.loadingSlots;
